#include "DXShadowMapFilterMaterial.h"
...
rendering::DXShadowMapFilterMaterial::DXShadowMapFilterMaterial(
    const rendering::DXShader& vertexShader,
    const rendering::DXShader& pixelShader,
    DXTexture* srcTex,
    DXTexture* dstTex) :
    DXMaterial(DXShadowMapFilterMaterialMeta::GetInstance(), vertexShader, pixelShader),
    m_srcTex(srcTex),
    m_dstTex(dstTex)
{
    CacheObjects();
...
ID3D12CommandList* rendering::DXShadowMapFilterMaterial::GenerateCommandList(
    ...
    UINT instanceIndex)
{
...

    {
        ...
        {
            CD3DX12_RESOURCE_BARRIER::CD3DX12_RESOURCE_BARRIER::Transition(m_srcTex->GetTexture(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
            CD3DX12_RESOURCE_BARRIER::CD3DX12_RESOURCE_BARRIER::Transition(m_dstTex->GetTexture(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET),
        };
    ...
    UINT64 height = m_dstTex->GetTextureDescription().Height;
...

    {
        CD3DX12_RESOURCE_BARRIER barrier[] =
        {
            CD3DX12_RESOURCE_BARRIER::CD3DX12_RESOURCE_BARRIER::Transition(m_srcTex->GetTexture(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_PRESENT),
            CD3DX12_RESOURCE_BARRIER::CD3DX12_RESOURCE_BARRIER::Transition(m_dstTex->GetTexture(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT)
        };
        commandList->ResourceBarrier(_countof(barrier), barrier);
    }
...
void rendering::DXShadowMapFilterMaterial::CreatePipelineStateAndRootSignature()
{
...
void rendering::DXShadowMapFilterMaterial::CreateDescriptorHeaps()
{
    {
        std::list<DXTexture*> textures;
        textures.push_back(m_srcTex);
